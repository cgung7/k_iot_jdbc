...
/*
    ...
    1. 필요한 준비물
        ...
        3) MySQL (MySQL Sever 8.x 버전 이상 권장)
        4) JDBC 드라이버
            : mysql-connector.java-x.x.xx.jar
            > 자바가 DB와 통신하도록 도와주는 드라이버 (연결 통로)
    ...
    3. JDBC 연결 흐름
        ...
        1) JDBC 드라이버 로그
            ...
            - Class.forName()을 사용해어 메모리에 적제

            Class.forName("com.mysql.cj.jdbc.Driver");

            >> Java는 다양한 DB와 연결할 수 있어야 한다 - 표준 인터페이스만 제공
                실제 동작은 DB 벤더가 만든 드라이버 구현체가 처리

        2) 데이터베이스 연결 (Connection)
            - 드라이버가 로드되면, DB에 연결 가능
            - DriverManager.getConnection()을 동해 Connection 객체 생성

            cf) Connection 객체
                - 실제 DB 서버와 연결된 통로를 의미
